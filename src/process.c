...
cc_oci_setup_shim (struct cc_oci_config *config,
			...
			int proxy_io_fd)
{
	struct termios  slave_term_settings;
	int             tty_fd;
	GArray         *fds;
	...
	if (config->oci.process.terminal) {
		...
		/* Docker sets the tty as raw. So Ctl-C does not generate signals
		 * Explicitly enable ISIG here to enable signal generating characters
		 */
		if (tcgetattr(tty_fd, &slave_term_settings) != 0) {
			g_warning("Not able to get the terminal settings: %s", strerror(errno));
			return false;
		}

		slave_term_settings.c_lflag |= ISIG;

		if (tcsetattr (tty_fd, TCSANOW, &slave_term_settings)) {
			g_warning("Not able to set terminal attributes: %s\n", strerror(errno));
			return false;
		}
